...
@section('content')
    <div class="container">
        <h1 class="display-1">@lang('lang.uploads_edit_heading')</h1>
        <form action="{{  route('upload.update', ['upload' => $upload->id]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <label class="d-block mb-1 mt-5 display-6 text-light" for="title">@lang('lang.article_title_english'):</label>
            <input  type=text placeholder="file name" name="title" value="{{ old('title', $upload->title) }}">
            @error('title')
                <span class="text-danger">{{ $message }}</span>
            @enderror

            <label class="d-block mb-1 mt-5 display-6 text-light" for="title_fr">@lang('lang.article_title_french'):</label>
            ...
            <div class="d-block mt-5 display-6">
                ...
                <span class="display-6 text-light" id="file-info">{{ $upload->file_path }}</span>
            </div>
            @error('file')
                <span class="text-danger">{{ $message }}</span>
            @enderror
